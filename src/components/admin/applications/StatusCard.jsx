          ...
          {editing ? (
            <div className="space-y-4">
              ...
              {formData.status === "rejected" && (
                <div className="mt-4 space-y-4">
                  ...
                  {/* Silver Medalist Checkbox */}
                  <div className="flex items-start gap-2 p-3 bg-blue-50 rounded-lg border border-blue-100">
                    <input
                      type="checkbox"
                      id="isSilverMedalist"
                      checked={formData.isSilverMedalist || false}
                      onChange={(e) => onFormChange("isSilverMedalist", e.target.checked)}
                      className="mt-1 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <label htmlFor="isSilverMedalist" className="text-sm cursor-pointer">
                      <span className="font-medium text-gray-800 block">
                        مرشح مميز (للمستقبل)
                      </span>
                      <span className="text-gray-500">
                        تحديد هذا المرشح كخيار بديل أو مناسب لفرص مستقبلية في حال توفر شواغر.
                      </span>
                    </label>
                  </div>
                </div>
            ...
            <div>
              ...
              {application?.isSilverMedalist && (
                <div className="mt-3 flex items-center gap-2 px-3 py-2 bg-blue-50 text-blue-800 rounded-md border border-blue-100 text-sm">
                  <Medal size={16} />
                  <span className="font-medium">مرشح مميز (محفوظ للمستقبل)</span>
                </div>
              )}
            </div>
          )}
